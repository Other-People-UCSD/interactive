...
/**
 * Builds the story in primarily sequential function calls.
 * To sequentially run the animation, each function must await on Promises to be resolved,
 * then the next function is able to execute to modify the next part of the story.
 * It is still possible to run multiple functions at once by omitting the await keyword.
 * @returns 0 on successful completion of the story
 */
...
function delPassage(id, delay, highlight=true) {
    return new Promise((resolve) => {
        ...
        setTimeout(() => { 
            if (id == 'output-text') {
                passage.innerHTML = '';
                passage.style.backgroundColor = inherit;
